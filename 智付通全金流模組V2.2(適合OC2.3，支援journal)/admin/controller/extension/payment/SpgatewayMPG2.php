...
class ControllerExtensionPaymentSpgatewayMPG2 extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			//$this->response->redirect($this->url->link('extension/extension/payment', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/SpgatewayMPG2', 'token=' . $this->session->data['token'], true)
		);
		
		# Set the form action
		//$data['SpgatewayMPG2_action'] = $this->url->link('payment/SpgatewayMPG2', 'token=' . $this->session->data['token'], 'SSL');
		$data['SpgatewayMPG2_action'] = $this->url->link('extension/payment/SpgatewayMPG2', 'token=' . $this->session->data['token'], true);
		
		# Set the cancel button
		//$data['SpgatewayMPG2_cancel'] = $this->url->link('extension/payment', 'token=' . $this->session->data['token'], 'SSL');
		$data['SpgatewayMPG2_cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'] . '&type=payment', true);
        ...
        $data['module_statuses'] = array(
            array(
                'value' => '1',
                'text' => $data['SpgatewayMPG2_text_enabled']
            ),
            array(
                'value' => '0',
                'text' => $data['SpgatewayMPG2_text_disabled']
            )
        );
