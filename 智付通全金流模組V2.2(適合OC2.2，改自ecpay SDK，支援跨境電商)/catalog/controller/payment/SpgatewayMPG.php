...
class ControllerPaymentSpgatewayMPG extends Controller {
	...
	public function index() {
		...
		$data = array();
		$data['SpgatewayMPG_action'] = $this->url->link('payment/SpgatewayMPG/redirect', '', 'SSL');
		
		# Get the translation
		$this->load->language('payment/SpgatewayMPG');
		$data['SpgatewayMPG_text_title'] = $this->config->get('SpgatewayMPG_storename');//$this->language->get('SpgatewayMPG_text_title');
		$data['SpgatewayMPG_text_credit'] = $this->language->get('SpgatewayMPG_text_credit');
		$data['SpgatewayMPG_text_detail'] = $this->language->get('SpgatewayMPG_text_detail');
		$data['SpgatewayMPG_text_payment_methods'] = $this->language->get('SpgatewayMPG_text_payment_methods');
		$data['SpgatewayMPG_text_checkout_button'] = $this->language->get('SpgatewayMPG_text_checkout_button');
		...
		foreach ($payment_methods as $payment_type => $value) {
			$data['payment_methods'][$payment_type] = $this->language->get('SpgatewayMPG_text_' . $value);
		}
		...
		$payment_template .= (strpos(VERSION, '2.2.') !== false) ? '/payment/SpgatewayMPG.tpl' : '/template/payment/SpgatewayMPG.tpl';*/

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/SpgatewayMPG.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/SpgatewayMPG.tpl', $data);
		} else {
			return $this->load->view('payment/SpgatewayMPG.tpl', $data);
		}
	...
	public function redirect() {
		try {
			# Load SpgatewayMPG translation
			$this->load->language('payment/SpgatewayMPG');
			
			# Validate the payment
			//$payment_type = "All";//$this->request->post['SpgatewayMPG_choose_payment'];
			$this->load->model('payment/SpgatewayMPG');
			//$is_valid = $this->model_payment_SpgatewayMPG->vaildatePayment($payment_type);
			//if (!$is_valid) {
			//	throw new Exception($this->language->get('SpgatewayMPG_error_invalid_payment'));
			//} else {
				if (!isset($this->session->data['order_id'])) {
					throw new Exception($this->language->get('SpgatewayMPG_error_order_id_miss'));
				} else {
					# Get the order info
					...
					$invoke_result = $this->model_payment_SpgatewayMPG->invokeSpgatewayMPGModule();
					if (!$invoke_result) {
						throw new Exception($this->language->get('SpgatewayMPG_error_module_miss'));
					} else {
						# Set SpgatewayMPG parameters
						$aio = new SpgatewayMPG_AllInOne();
						$aio->Send['MerchantTradeNo'] = '';
						$service_url = '';
						$expiredate = $this->config->get('SpgatewayMPG_expiredate'); //抽取非即時付款的繳費時間

						$aio->MerchantID = $this->config->get('SpgatewayMPG_merchant_id');
						if ($this->config->get('SpgatewayMPG_test_mode')) {//$this->model_payment_SpgatewayMPG->isTestMode($aio->MerchantID)) {
							$service_url = 'https://ccore.spgateway.com/MPG/mpg_gateway';//'https://payment-stage.SpgatewayMPG.com.tw/Cashier/AioCheckOut';
							//$aio->Send['MerchantTradeNo'] = date('YmdHis');
						} else {
							$service_url = 'https://core.spgateway.com/MPG/mpg_gateway';//'https://payment.SpgatewayMPG.com.tw/Cashier/AioCheckOut';
						}
						$aio->HashKey = $this->config->get('SpgatewayMPG_hash_key');
						$aio->HashIV = $this->config->get('SpgatewayMPG_hash_iv');
						$aio->ServiceURL = $service_url;
						$aio->Send['NotifyURL'] 		= $this->url->link('payment/SpgatewayMPG/feedback', '', 'SSL');//$this->url->link('payment/SpgatewayMPG/response', '', 'SSL');
						$aio->Send['ClientBackURL'] 	= $this->url->link('common/home', '', 'SSL');//str_replace('&amp;', '&', $this->url->link('account/order/info', 'order_id=' . $order_id, 'SSL'));
						$aio->Send['ReturnURL'] 		= $this->url->link('payment/SpgatewayMPG/feedbackShow', '', 'SSL');
						...
						$aio->Send['ExpireDate'] 		= date("Ymd",strtotime("+ $expiredate day")); //繳費期限三天
						$aio->Send['CustomerURL'] 		= $this->url->link('payment/SpgatewayMPG/feedback', '', 'SSL');

						#API設定付款方法
						$payment_methods = $this->config->get('SpgatewayMPG_payment_methods');
						//$payment_desc = $this->language->get('SpgatewayMPG_text_' . $payment_methods[$payment_type]);
						foreach ($payment_methods as $payment_type => $payment_value) { 
							$aio->Send[$payment_type] = 1;
							//$payment_type; echo $payment_desc; 
						} 
						//API設定滿額開啟分期
						/*if ( $this->model_payment_SpgatewayMPG->formatOrderTotal($order['total']) >= $this->config->get('SpgatewayMPG_insflagday') ){
							$aio->Send['InstFlag'] = $this->config->get('SpgatewayMPG_insflagmode');
						}*/
						$InstFlag =array(
							"3"=>'',
							"6"=>'',
						...
						if (!empty($this->config->get('SpgatewayMPG_insflagday_3')) && ($this->model_payment_SpgatewayMPG->formatOrderTotal($order['total']) >= $this->config->get('SpgatewayMPG_insflagday_3'))){
							$InstFlag['3'] = true;//開啟3期
						}
						if (!empty($this->config->get('SpgatewayMPG_insflagday_6')) && ($this->model_payment_SpgatewayMPG->formatOrderTotal($order['total']) >= $this->config->get('SpgatewayMPG_insflagday_6'))){
							$InstFlag['6'] = true;//開啟6期

						}
						if (!empty($this->config->get('SpgatewayMPG_insflagday_12')) && ($this->model_payment_SpgatewayMPG->formatOrderTotal($order['total']) >= $this->config->get('SpgatewayMPG_insflagday_12'))){
							$InstFlag['12'] = true;//開啟12期
						}
						if (!empty($this->config->get('SpgatewayMPG_insflagday_18')) && ($this->model_payment_SpgatewayMPG->formatOrderTotal($order['total']) >= $this->config->get('SpgatewayMPG_insflagday_18'))){
							$InstFlag['18'] = true;//開啟18期
						}
						if (!empty($this->config->get('SpgatewayMPG_insflagday_24')) && ($this->model_payment_SpgatewayMPG->formatOrderTotal($order['total']) >= $this->config->get('SpgatewayMPG_insflagday_24'))){
							$InstFlag['24'] = true;//開啟24期
						}
						if (!empty($this->config->get('SpgatewayMPG_insflagday_30')) && ($this->model_payment_SpgatewayMPG->formatOrderTotal($order['total']) >= $this->config->get('SpgatewayMPG_insflagday_30'))){
							$InstFlag['30'] = true;//開啟30期
						}
						$InstFlag_push ='';
						...
						$InstFlag_push = substr($InstFlag_push,1);//移除第一個逗號
						$aio->Send['InstFlag'] = $InstFlag_push;//輸入要給藍新的值

						
						# Set the product info
						$aio->Send['Amt'] = $this->model_payment_SpgatewayMPG->formatOrderTotal($order['total']);
						array_push(
							$aio->Send['Items'],
							array(
								'Name' => $productsall,//$this->language->get('SpgatewayMPG_text_item_name'),
								'Price' => $aio->Send['Amt'],
						...
						$aio->Send['TradeDesc'] = 'SpgatewayMPG_module_opencart_1.0.1114';
						...
						/*switch ($aio->Send['ChoosePayment']) {
							case SpgatewayMPG_PaymentMethod::Credit:
								...
								break;
							case SpgatewayMPG_PaymentMethod::WebATM:
								break;
							case SpgatewayMPG_PaymentMethod::ATM:
								//$aio->SendExtend['ExpireDate'] = 3;
								$aio->SendExtend['PaymentInfoURL'] = $aio->Send['ReturnURL'];
								break;
							case SpgatewayMPG_PaymentMethod::CVS:
	                            ...
	                            $aio->SendExtend['PaymentInfoURL'] = $aio->Send['ReturnURL'];
	                            break;
							case SpgatewayMPG_PaymentMethod::BARCODE:
					...
					$payment_methods = $this->config->get('SpgatewayMPG_payment_methods');
					$order_create_status_id = $this->config->get('SpgatewayMPG_create_status');
					//$payment_desc = $this->language->get('SpgatewayMPG_text_' . $payment_methods[$payment_type]);
					$payment_desc2 = $this->language->get('SpgatewayMPG_text_createstatus');
        ...
        if (isset($result['Status']) && !empty($result['Status'])) {
            ...
            //if ($this->customer->isLogged()) {
                ...
                if (in_array($result['Status'], array('SUCCESS', 'CUSTOM'))) {
                	...
                	//$show_text_customer .= '<a href="https://ccore.spgateway.com/barcode?MerID=' . $this->config->get('SpgatewayMPG_merchant_id') . '&MerTradeNo=' . $result['MerchantOrderNo'] . '"> 列印繳款單 </a>';
            ...
            //}
                if ($result['PaymentType'] == 'BARCODE') {
            		...
            		if ($this->config->get('SpgatewayMPG_test_mode')) {
						$service_urlcode = 'https://ccore.spgateway.com/barcode?MerID=';
					} else {
						$service_urlcode = 'https://core.spgateway.com/barcode?MerID=';
					}

	            	$data['continue'] = $service_urlcode . $this->config->get('SpgatewayMPG_merchant_id') . '&MerTradeNo=' . $result['MerchantOrderNo'];
                }else{
			...
			/*$ServiceURL = $this->url->link('payment/SpgatewayMPG/feedback', '', 'SSL');
	        $szHtml =  '<!DOCTYPE html>';
	        $szHtml .= '<html>';
	        $szHtml .=     '<head>';
    ...
    public function feedback() {
        ...
        if (!empty($order_info)) {
            ...
            $store_info = $this->model_setting_setting->getSetting('SpgatewayMPG', '0'); //抽取預設商店的後台值
            ...
            if (in_array($result['Status'], array('SUCCESS', 'CUSTOM'))) {
                ...
                if (intval($order_info['total']) == $result['Amt']) {
                    ...
                    if ($checkCode == $result['CheckCode']) {
                       ...
                       // if ($order_info['order_status_id'] != $store_info['SpgatewayMPG_order_finish_status_id']) {
                       	if ($order_info['order_status_id'] != $store_info['SpgatewayMPG_success_status']) {
                            ...
                            $newstatus = (isset($result['ExpireDate'])) ? $store_info['SpgatewayMPG_getcode_status'] : $store_info['SpgatewayMPG_success_status'];
                            //$comment =  $this->_getComment($result);
                            $this->model_checkout_order->addOrderHistory(
											(int) $order_info['order_id'],
											$newstatus,
											//$store_info['SpgatewayMPG_success_status'],
											$comment,
											true
										);
                ...
                if (in_array($result['PaymentType'], array('CREDIT', 'WEBATM'))) {
	                ...
	                $this->model_checkout_order->addOrderHistory(
					(int) $order_info['order_id'],
					$this->config->get('SpgatewayMPG_failed_status'),
					$comment,
					true
					);
                }
        ...
        if (isset($result['ExpireDate'])) {
			$ServiceURL = $this->url->link('payment/SpgatewayMPG/feedbackCode', '', 'SSL');
	        $szHtml =  '<!DOCTYPE html>';
	        $szHtml .= '<html>';
	        $szHtml .=     '<head>';
	        $szHtml .=         '<meta charset="utf-8">';
	...
	/*public function response() {
		...
		try {
			...
			$invoke_result = $this->model_payment_SpgatewayMPG->invokeSpgatewayMPGModule();
			if (!$invoke_result) {
				throw new Exception('SpgatewayMPG module is missing.');
			} else {
				$aio = new SpgatewayMPG_AllInOne();
				$aio->HashKey = $this->config->get('SpgatewayMPG_hash_key');
				$aio->HashIV = $this->config->get('SpgatewayMPG_hash_iv');
				$SpgatewayMPG_feedback = $aio->CheckOutFeedback();
				unset($aio);
				
				# Process SpgatewayMPG feedback
				if(count($SpgatewayMPG_feedback) < 1) {
					throw new Exception('Get SpgatewayMPG feedback failed.');
				} else {
					# Get the cart order id
					$cart_order_id = $this->model_payment_SpgatewayMPG->getCartOrderID($SpgatewayMPG_feedback['MerchantTradeNo'], $this->config->get('SpgatewayMPG_merchant_id'));
				
					# Get the cart order amount
					$order = $this->model_checkout_order->getOrder($cart_order_id);
					$cart_amount = $this->model_payment_SpgatewayMPG->formatOrderTotal($order['total']);
					
					# Check the amounts
					$SpgatewayMPG_amount = $SpgatewayMPG_feedback['TradeAmt'];
					if ($cart_amount != $SpgatewayMPG_amount) {
						throw new Exception(sprintf('Order %s amount are not identical.', $cart_order_id));
					} else {
						# Set the common comments
						$comments = sprintf(
							$this->language->get('SpgatewayMPG_text_common_comments'),
							$SpgatewayMPG_feedback['PaymentType'],
							$SpgatewayMPG_feedback['TradeDate']
						);
						
						# Set the getting code comments
						$return_message = $SpgatewayMPG_feedback['RtnMsg'];
						$return_code = $SpgatewayMPG_feedback['RtnCode'];
						$get_code_result_comments = sprintf(
							$this->language->get('SpgatewayMPG_text_get_code_result_comments'),
							$return_code,
							$return_message
						);
						...
						$payment_result_comments = sprintf(
							$this->language->get('SpgatewayMPG_text_payment_result_comments'),
							$return_code,
							$return_message
						);
						
						# Get SpgatewayMPG payment method
						$type_pieces = explode('_', $SpgatewayMPG_feedback['PaymentType']);
						$SpgatewayMPG_payment_method = $type_pieces[0];
						
						# Update the order status and comments
						$fail_message = sprintf('Order %s Exception.(%s: %s)', $cart_order_id, $return_code, $return_message);
						$order_create_status_id = $this->config->get('SpgatewayMPG_create_status');
						$paid_succeeded_status_id = $this->config->get('SpgatewayMPG_success_status');
						
						switch($SpgatewayMPG_payment_method) {
							case SpgatewayMPG_PaymentMethod::Credit:
							case SpgatewayMPG_PaymentMethod::WebATM:
								if ($return_code != 1 and $return_code != 800) {
									...
									if ($order['order_status_id'] != $order_create_status_id) {
										...
										// 判斷電子發票是否啟動 START
										$nInvoice_Status  = $this->config->get('SpgatewayMPGinvoice_status');
										if($nInvoice_Status == 1)
										{
											$this->load->model('payment/SpgatewayMPGinvoice');
											$nInvoice_Autoissue 	= $this->config->get('SpgatewayMPGinvoice_autoissue');
											$sCheck_Invoice_SDK	= $this->model_payment_SpgatewayMPGinvoice->check_invoice_sdk();
											if( $nInvoice_Autoissue == 1 && $sCheck_Invoice_SDK != false )
											{	
												$this->model_payment_SpgatewayMPGinvoice->createInvoiceNo($cart_order_id, $sCheck_Invoice_SDK);
											}
										}
										// 判斷電子發票是否啟動 END
							...
							case SpgatewayMPG_PaymentMethod::ATM:
								if ($return_code != 1 and $return_code != 2 and $return_code != 800) {
									...
									if ($return_code == 2) {
										...
										$comments .= sprintf(
											$this->language->get('SpgatewayMPG_text_atm_comments'),
											$SpgatewayMPG_feedback['BankCode'],
											$SpgatewayMPG_feedback['vAccount'],
											$SpgatewayMPG_feedback['ExpireDate']
										);
										$this->model_checkout_order->addOrderHistory(
											$cart_order_id,
							...
							case SpgatewayMPG_PaymentMethod::CVS:
								if ($return_code != 1 and $return_code != 800 and $return_code != 10100073) {
									...
									if ($return_code == 10100073) {
										$comments .= sprintf(
											$this->language->get('SpgatewayMPG_text_cvs_comments'),
											$SpgatewayMPG_feedback['PaymentNo'],
											$SpgatewayMPG_feedback['ExpireDate']
										);
										$this->model_checkout_order->addOrderHistory(
											$cart_order_id,
							...
							case SpgatewayMPG_PaymentMethod::BARCODE:
								if ($return_code != 1 and $return_code != 800 and $return_code != 10100073) {
									...
									if ($return_code == 10100073) {
										$comments .= sprintf(
											$this->language->get('SpgatewayMPG_text_barcode_comments'),
											$SpgatewayMPG_feedback['ExpireDate'],
											$SpgatewayMPG_feedback['Barcode1'],
											$SpgatewayMPG_feedback['Barcode2'],
											$SpgatewayMPG_feedback['Barcode3']
										);
										$this->model_checkout_order->addOrderHistory(
											$cart_order_id,
											$order_create_status_id,
			...
			$error = $e->getMessage();
			if (!empty($order)) {
				$paid_failed_status_id = $this->config->get('SpgatewayMPG_failed_status');
				$comments = sprintf($this->language->get('SpgatewayMPG_text_failure_comments'), $error);
		...
		# Return URL log
		$this->model_payment_SpgatewayMPG->logMessage('Order ' . $cart_order_id . ' process SpgatewayMPG response result : ' . $result_message);
    ...
    protected function _updateOrder($order_info, $result, $store_info) {
        ...
        // 訂單完成狀態
        $order_status_id = in_array($result['Status'], array('SUCCESS', 'CUSTOM')) ? (int) $store_info['SpgatewayMPG_success_status'] : (int) $store_info['SpgatewayMPG_failed_status'];
						//$order_create_status_id = $this->config->get('SpgatewayMPG_create_status');
						//$paid_succeeded_status_id = $this->config->get('SpgatewayMPG_success_status');
        // 訂單備註
