 ...
 * 付款方式。
 */
abstract class SpgatewayMPG_PaymentMethod {
 ...
 * 付款方式子項目。
 */
abstract class SpgatewayMPG_PaymentMethodItem {
 ...
 * 額外付款資訊。
 */
abstract class SpgatewayMPG_ExtraPaymentInfo {
 ...
 * 額外付款資訊。
 */
abstract class SpgatewayMPG_DeviceType {
 ...
 * 信用卡訂單處理動作資訊。
 */
abstract class SpgatewayMPG_ActionType {
 ...
 * 定期定額的週期種類。
 */
abstract class SpgatewayMPG_PeriodType {
 ...
 * 電子發票開立註記。
 */
abstract class SpgatewayMPG_InvoiceState {
    /**
     * 需要開立電子發票。
     */
    const Yes = 'Y';
 ...
 * 電子發票載具類別
 */
abstract class SpgatewayMPG_CarruerType {
  // 無載具
  const None = '';
 ...
 * 電子發票列印註記
 */
abstract class SpgatewayMPG_PrintMark {
  // 不列印
  const No = '0';
 ...
 * 電子發票捐贈註記
 */
abstract class SpgatewayMPG_Donation {
  // 捐贈
  const Yes = '1';
 ...
 * 通關方式
 */
abstract class SpgatewayMPG_ClearanceMark {
  // 經海關出口
  const Yes = '1';
 ...
 * 課稅類別
 */
abstract class SpgatewayMPG_TaxType {
  // 應稅
  const Dutiable = '1';
 ...
 * 字軌類別
 */
abstract class SpgatewayMPG_InvType {
  // 一般稅額
  const General = '07';
...
abstract class SpgatewayMPG_EncryptType {
    // MD5(預設)
    const ENC_MD5 = 0;
...
class SpgatewayMPG_AllInOne {
    ...
    public $EncryptType = SpgatewayMPG_EncryptType::ENC_MD5;
    ...
    function __construct() {
        ...
        $this->Send = array(
            ...
            "Amt"               => '',
            "TradeDesc"         => '',
            //"ChoosePayment"     => SpgatewayMPG_PaymentMethod::ALL,
            "Remark"            => '',
            "ChooseSubPayment"  => SpgatewayMPG_PaymentMethodItem::None,
            "NeedExtraPaidInfo" => SpgatewayMPG_ExtraPaymentInfo::No,
            "DeviceSource"      => '',
            "IgnorePayment"     => '',
            "PlatformID"        => '',
            "InvoiceMark"       => SpgatewayMPG_InvoiceState::No,
            "Items"             => array(),
            "StoreID"           => '',
            "CustomField1"      => '',
            "CustomField2"      => '',
        ...
        $this->Action = array(
            'MerchantOrderNo' => '',
            'TradeNo' => '',
            'Action' => SpgatewayMPG_ActionType::C,
            'Amt' => 0
        );
        $this->Capture = array(
            'MerchantOrderNo' => '',
    ...
    function CheckOut($target = "_self") {
        ...
        SpgatewayMPG_Send::CheckOut($target,$arParameters,$this->SendExtend,$this->HashKey,$this->HashIV,$this->ServiceURL);
    }

    //產生訂單html code
    function CheckOutString($paymentButton = null, $target = "_self") {
        $arParameters = array_merge( array('MerchantID' => $this->MerchantID, 'EncryptType' => $this->EncryptType) ,$this->Send);
        return SpgatewayMPG_Send::CheckOutString($paymentButton,$target = "_self",$arParameters,$this->SendExtend,$this->HashKey,$this->HashIV,$this->ServiceURL);
    }

    //取得付款結果通知的方法
    function CheckOutFeedback() {
        return $arFeedback = SpgatewayMPG_CheckOutFeedback::CheckOut(array_merge($_POST, array('EncryptType' => $this->EncryptType)),$this->HashKey,$this->HashIV,0);   
    }

    //訂單查詢作業
    function QueryTradeInfo() {
        return $arFeedback = SpgatewayMPG_QueryTradeInfo::CheckOut(array_merge($this->Query,array('MerchantID' => $this->MerchantID, 'EncryptType' => $this->EncryptType)) ,$this->HashKey ,$this->HashIV ,$this->ServiceURL) ;
    }
    
    //信用卡定期定額訂單查詢的方法
    function QueryPeriodCreditCardTradeInfo() {
        return $arFeedback = SpgatewayMPG_QueryPeriodCreditCardTradeInfo::CheckOut(array_merge($this->Query,array('MerchantID' => $this->MerchantID, 'EncryptType' => $this->EncryptType)) ,$this->HashKey ,$this->HashIV ,$this->ServiceURL);
    }

    //信用卡關帳/退刷/取消/放棄的方法
    function DoAction() {
        return $arFeedback = SpgatewayMPG_DoAction::CheckOut(array_merge($this->Action,array('MerchantID' => $this->MerchantID, 'EncryptType' => $this->EncryptType)) ,$this->HashKey ,$this->HashIV ,$this->ServiceURL);
    }
        
    //合作特店申請撥款
    function AioCapture(){
        return $arFeedback = SpgatewayMPG_AioCapture::Capture(array_merge($this->Capture,array('MerchantID' => $this->MerchantID, 'EncryptType' => $this->EncryptType)) ,$this->HashKey ,$this->HashIV ,$this->ServiceURL);
    }
    ...
    function TradeNoAio($target = "_self"){
        ...
        SpgatewayMPG_TradeNoAio::CheckOut($target,$arParameters,$this->HashKey,$this->HashIV,$this->ServiceURL);
    }

    //查詢信用卡單筆明細紀錄
    function QueryTrade(){
        return $arFeedback = SpgatewayMPG_QueryTrade::CheckOut(array_merge($this->Trade,array('MerchantID' => $this->MerchantID, 'EncryptType' => $this->EncryptType)) ,$this->HashKey ,$this->HashIV ,$this->ServiceURL);
    }
    ...
    function FundingReconDetail($target = "_self"){
        ...
        SpgatewayMPG_FundingReconDetail::CheckOut($target,$arParameters,$this->HashKey,$this->HashIV,$this->ServiceURL);
    }
...
* 抽象類
*/
abstract class SpgatewayMPG_Aio
{
...
*  產生訂單
*/
class SpgatewayMPG_Send extends SpgatewayMPG_Aio
{   
    //付款方式物件
    ...
    {
        //宣告付款方式物件
        $PaymentMethod    = 'SpgatewayMPG_'.'ALL';//$arParameters['ChoosePayment'];
        ...
        //檢查跨境電商商品長度
        $arParameters = self::$PaymentObj->check_goods_alipay($arParameters);
    ...
    static function CheckOut($target = "_self",$arParameters = array(),$arExtend = array(),$HashKey='',$HashIV='',$ServiceURL=''){
        ...
        //產生檢查碼
        $szCheckValue = SpgatewayMPG_CheckValue::generate($arParameters,$HashKey,$HashIV,$arParameters['EncryptType']);
       //$szTradeInfo = SpgatewayMPG_CheckValue::gen_AES($arParameters,$HashKey,$HashIV);
        //生成表單，自動送出
        $szHtml =  '<!DOCTYPE html>';
        $szHtml .= '<html>';
        $szHtml .=     '<head>';
        ...
        $szHtml .=     '</head>';
        $szHtml .=     '<body>';
        $szHtml .=         "<form id=\"__SpgatewayMPGForm\" method=\"post\" target=\"{$target}\" action=\"{$ServiceURL}\">";
        ...
        $szHtml .=         '</form>';
        $szHtml .=         '<script type="text/javascript">document.getElementById("__SpgatewayMPGForm").submit();</script>';
        $szHtml .=     '</body>';
        $szHtml .= '</html>';
...
class SpgatewayMPG_CheckOutFeedback extends SpgatewayMPG_Aio 
{
    static function CheckOut($arParameters = array(),$HashKey = '' ,$HashIV = ''){
        ...
        $CheckValue = SpgatewayMPG_CheckValue::generate($arParameters,$HashKey,$HashIV,$EncryptType);
...
class SpgatewayMPG_QueryTradeInfo extends SpgatewayMPG_Aio
{
    static function CheckOut($arParameters = array(),$HashKey ='',$HashIV ='',$ServiceURL = ''){
        ...
        // 呼叫查詢。
        if (sizeof($arErrors) == 0) {
            $arParameters["CheckValue"] = SpgatewayMPG_CheckValue::generate($arParameters,$HashKey,$HashIV,$EncryptType);
            // 送出查詢並取回結果。
            ...
            if (sizeof($arFeedback) > 0) {
                $szConfirmMacValue = SpgatewayMPG_CheckValue::generate($arConfirmArgs,$HashKey,$HashIV,$EncryptType);
...
class SpgatewayMPG_QueryPeriodCreditCardTradeInfo extends SpgatewayMPG_Aio
{
    static function CheckOut($arParameters = array(),$HashKey ='',$HashIV ='',$ServiceURL = ''){
        $arErrors = array();
        $arParameters['TimeStamp'] = time();
...
class SpgatewayMPG_DoAction extends SpgatewayMPG_Aio
{
    static function CheckOut($arParameters = array(),$HashKey ='',$HashIV ='',$ServiceURL = ''){
                // 變數宣告。
        $arErrors = array();
        ...
        //產生驗證碼
        $szCheckValue = SpgatewayMPG_CheckValue::generate($arParameters,$HashKey,$HashIV,$EncryptType);
...
class SpgatewayMPG_AioCapture extends SpgatewayMPG_Aio
{
...
class SpgatewayMPG_TradeNoAio extends SpgatewayMPG_Aio
{   
...
class SpgatewayMPG_QueryTrade extends SpgatewayMPG_Aio
{
    static function CheckOut($arParameters = array(),$HashKey ='',$HashIV ='',$ServiceURL = ''){
        $arErrors = array();
        $arFeedback = array();
...
class SpgatewayMPG_FundingReconDetail extends SpgatewayMPG_Aio
{   
    static function CheckOut($target = "_self",$arParameters = array(),$HashKey='',$HashIV='',$ServiceURL=''){
        //產生檢查碼
        $EncryptType = $arParameters["EncryptType"];
...
Abstract class SpgatewayMPG_Verification
{
    // 電子發票延伸參數。
    ...
    //檢查兩岸支付通產品格式
    public function check_goods_alipay($arParameters = array()){
        // 檢查產品名稱。
        $szItemDesc = $arParameters['Title1'];
        $arErrors   = array();
        if (sizeof($arParameters['Title1']) > 0) {
            if (strlen($szItemDesc) > 0) {
                $szItemDesc = mb_substr($szItemDesc, 0, 35,"utf-8");
                $arParameters['Title1'] = $szItemDesc ;
            }
        } else {
            array_push($arErrors, "Goods information not found.");
            $arParameters['Title1'] = "Products" ; #使用預設值，避免報錯
        }

        if(sizeof($arErrors)>0) throw new Exception(join('<br>', $arErrors));

        #unset($arParameters['Title1']);
        return $arParameters ;
    }
    ...
    public function check_invoiceString($arExtend = array()){
        ...
        if(!array_key_exists('CarruerType', $arExtend)){
            $arExtend['CarruerType'] = SpgatewayMPG_CarruerType::None ;
        }else{
            //有設定統一編號的話，載具類別不可為合作特店載具或自然人憑證載具。
            $notPrint = array(SpgatewayMPG_CarruerType::Member, SpgatewayMPG_CarruerType::Citizen);
        ...
        if(!array_key_exists('CustomerID', $arExtend)) {
            ...
            if($arExtend['CarruerType'] == SpgatewayMPG_CarruerType::Member && strlen($arExtend['CustomerID']) == 0 ){
                array_push($arErrors, "CustomerID is required.");
            }
        }
        // 捐贈註記 Donation(預設為No)
        if(!array_key_exists('Donation', $arExtend)){
            $arExtend['Donation'] = SpgatewayMPG_Donation::No ;
        }else{
            //若有帶統一編號，不可捐贈
            if(strlen($arExtend['CustomerIdentifier']) > 0 && $arExtend['Donation'] != SpgatewayMPG_Donation::No){
        ...
        if(!array_key_exists('Print', $arExtend)){
            $arExtend['Print'] = SpgatewayMPG_PrintMark::No;
        }else{
            //捐贈註記為捐贈(Yes)時，請設定不列印(No)
            if($arExtend['Donation'] == SpgatewayMPG_Donation::Yes && $arExtend['Print'] != SpgatewayMPG_PrintMark::No){
                array_push($arErrors, "Print should be No.");
            }
            // 統一編號不為空字串時，請設定列印(Yes)
            if(strlen($arExtend['CustomerIdentifier']) > 0 && $arExtend['Print'] != SpgatewayMPG_PrintMark::Yes){
        ...
        if(!array_key_exists('CustomerName', $arExtend)){
            ...
            if($arExtend['Print'] == SpgatewayMPG_PrintMark::Yes && strlen($arExtend['CustomerName']) == 0){
        ...
        if(!array_key_exists('CustomerAddr', $arExtend)){
            ...
            // 列印註記為列印(Yes)時，此參數不可為空字串
            if($arExtend['Print'] == SpgatewayMPG_PrintMark::Yes && strlen($arExtend['CustomerAddr']) == 0){
        ...
        if(!array_key_exists('ClearanceMark', $arExtend)) {
            ...
            if($arExtend['TaxType'] == SpgatewayMPG_TaxType::Zero && ($arExtend['ClearanceMark'] != SpgatewayMPG_ClearanceMark::Yes || $arExtend['ClearanceMark'] != SpgatewayMPG_ClearanceMark::No)) {
                array_push($arErrors, "ClearanceMark is required.");
            }
            if (strlen($arExtend['ClearanceMark']) > 0 && $arExtend['TaxType'] != SpgatewayMPG_TaxType::Zero) {
        ...
        if (!array_key_exists('CarruerNum', $arExtend)) {
            ...
            switch ($arExtend['CarruerType']) {
                ...
                case SpgatewayMPG_CarruerType::None:
                case SpgatewayMPG_CarruerType::Member:
                break;
                // 載具類別為買受人自然人憑證(Citizen)時，請設定自然人憑證號碼，前2碼為大小寫英文，後14碼為數字
                case SpgatewayMPG_CarruerType::Citizen:
                ...
                case SpgatewayMPG_CarruerType::Cellphone:
        ...
        if ($arExtend['Donation'] == SpgatewayMPG_Donation::Yes) {
        ...
        if(!array_key_exists('InvoiceItems', $arExtend)){
            ...
            if ($arExtend['TaxType'] == SpgatewayMPG_TaxType::Mix) {
                if (in_array(SpgatewayMPG_TaxType::Dutiable, $tmpItemTaxType) and in_array(SpgatewayMPG_TaxType::Free, $tmpItemTaxType)) {
                    // Do nothing
                }  else {
                    $tmpItemTaxType = array();
                }
...
*  付款方式：超商代碼
*/
class SpgatewayMPG_CVS extends SpgatewayMPG_Verification
{
    public  $arPayMentExtend = array(
                            'Desc_1'           =>'',
                            'Desc_2'           =>'',
...
* 付款方式 : BARCODE 
*/
class SpgatewayMPG_BARCODE extends SpgatewayMPG_Verification
{
    public  $arPayMentExtend = array(
                            'Desc_1'           =>'',
                            'Desc_2'           =>'',
...
*  付款方式 ATM
*/
class SpgatewayMPG_ATM extends SpgatewayMPG_Verification
{
    public  $arPayMentExtend = array(
                            'ExpireDate'       => 3,
                            'PaymentInfoURL'   => '',
...
*  付款方式 WebATM
*/
class SpgatewayMPG_WebATM extends SpgatewayMPG_Verification
{
    public  $arPayMentExtend = array();
...
* 付款方式 : 信用卡
*/
class SpgatewayMPG_Credit extends SpgatewayMPG_Verification
{
    public $arPayMentExtend = array(
                                    "CreditInstallment" => '',
                                    "InstallmentAmount" => 0,
...
*  付款方式：全功能
*/
class SpgatewayMPG_ALL extends SpgatewayMPG_Verification
{
    public  $arPayMentExtend = array();
...
* 付款方式 : Android Pay
*/
class SpgatewayMPG_AndroidPay extends SpgatewayMPG_Verification
{
    public $arPayMentExtend = array();
...
class SpgatewayMPG_CheckValue{
    ...
    static function generate($arParameters = array(),$HashKey = '' ,$HashIV = '',$encType = 0){
        ...
        {   
            ...
            uksort($arParameters, array('SpgatewayMPG_CheckValue','merchantSort'));
            ...
            // 編碼
            switch ($encType) {
                case SpgatewayMPG_EncryptType::ENC_SHA256:
                    // SHA256 編碼
                    $sMacValue = hash('sha256', $sMacValue);
                break;
                
                case SpgatewayMPG_EncryptType::ENC_MD5:
                default:
                // MD5 編碼
