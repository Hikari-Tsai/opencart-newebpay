...
class ControllerPaymentSpgatewayMPG2 extends Controller {
	...
	public function index() {
		...
		$data = array();
		$data['SpgatewayMPG2_action'] = $this->url->link('payment/SpgatewayMPG2/redirect', '', 'SSL');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/SpgatewayMPG2.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/SpgatewayMPG2.tpl', $data);
		} else {
			return $this->load->view('payment/SpgatewayMPG2.tpl', $data);
		}
			...
			//} else {
				if (!isset($this->session->data['order_id'])) {
					...
					if (!$invoke_result) {
						...
						$service_url = '';
						$expiredate = $this->config->get('SpgatewayMPG2_expiredate'); //抽取非即時付款的繳費時間
						...
						$aio->Send['NotifyURL'] 		= $this->url->link('payment/SpgatewayMPG2/feedback', '', 'SSL');//$this->url->link('payment/SpgatewayMPG2/response', '', 'SSL');
						$aio->Send['ClientBackURL'] 	= $this->url->link('common/home', '', 'SSL');//str_replace('&amp;', '&', $this->url->link('account/order/info', 'order_id=' . $order_id, 'SSL'));
						$aio->Send['ReturnURL'] 		= $this->url->link('payment/SpgatewayMPG2/feedbackShow', '', 'SSL');
						...
						$aio->Send['CustomerURL'] 		= $this->url->link('payment/SpgatewayMPG2/feedback', '', 'SSL');
						...
						if (!empty($this->config->get('SpgatewayMPG2_insflagday_3')) && ($this->model_payment_SpgatewayMPG2->formatOrderTotal($order['total']) >= $this->config->get('SpgatewayMPG2_insflagday_3'))){
							$InstFlag['3'] = true;//開啟3期
						}
						if (!empty($this->config->get('SpgatewayMPG2_insflagday_6')) && ($this->model_payment_SpgatewayMPG2->formatOrderTotal($order['total']) >= $this->config->get('SpgatewayMPG2_insflagday_6'))){
							$InstFlag['6'] = true;//開啟6期

						}
						if (!empty($this->config->get('SpgatewayMPG2_insflagday_12')) && ($this->model_payment_SpgatewayMPG2->formatOrderTotal($order['total']) >= $this->config->get('SpgatewayMPG2_insflagday_12'))){
							$InstFlag['12'] = true;//開啟12期
						}
						if (!empty($this->config->get('SpgatewayMPG2_insflagday_18')) && ($this->model_payment_SpgatewayMPG2->formatOrderTotal($order['total']) >= $this->config->get('SpgatewayMPG2_insflagday_18'))){
							$InstFlag['18'] = true;//開啟18期
						}
						if (!empty($this->config->get('SpgatewayMPG2_insflagday_24')) && ($this->model_payment_SpgatewayMPG2->formatOrderTotal($order['total']) >= $this->config->get('SpgatewayMPG2_insflagday_24'))){
							$InstFlag['24'] = true;//開啟24期
						}
						if (!empty($this->config->get('SpgatewayMPG2_insflagday_30')) && ($this->model_payment_SpgatewayMPG2->formatOrderTotal($order['total']) >= $this->config->get('SpgatewayMPG2_insflagday_30'))){
						...
						$aio->Send['Amt'] = $this->model_payment_SpgatewayMPG2->formatOrderTotal($order['total']);
						array_push(
							$aio->Send['Items'],
							array(
						...
						$aio->Send['Count']			=1;
						$aio->Send['Pid(Count)']	=;
						$aio->Send['Title(Count)']	=;
						$aio->Send['Desc(Count)']	=;
						$aio->Send['Price(Count)']	=;
						$aio->Send['Qty(Count)']	=;*/
    ...
    public function feedback() {
        ...
        if (!empty($order_info)) {
            ...
            $store_info = $this->model_setting_setting->getSetting('SpgatewayMPG2', '0'); //抽取預設商店的後台值
        ...
        if (isset($result['ExpireDate'])) {
			$ServiceURL = $this->url->link('payment/SpgatewayMPG2/feedbackCode', '', 'SSL');
	        $szHtml =  '<!DOCTYPE html>';
	        $szHtml .= '<html>';
	        $szHtml .=     '<head>';
	        $szHtml .=         '<meta charset="utf-8">';
